...
{
    public class DryadSpawnState : DryadBaseState
    {
        float stateTime = 2f;
        float stateTimer;

        public DryadSpawnState(DryadStateMachine stateMachine)
            : base(stateMachine) { }
        ...
        public override void Tick(float deltaTime)
        {
            ...
            if (stateTimer <= 0f)
            {
                stateMachine.SwitchState(new DryadAttackingState(stateMachine));
                return;
            }
        }
    }
